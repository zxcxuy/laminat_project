...
from insertData import insertData, insertColl
import os
    ...
    if not os.path.exists(txt_filename):
        with open(txt_filename, 'w', encoding='utf-8') as f:
            for idx, link in enumerate(links, 1):
                f.write(f"{idx}. {link[0]}\n")
        print(f"Создан файл {txt_filename} с {len(links)} ссылками")
    else:
        print(f"Файл {txt_filename} уже существует")

def parseColl(links, driver, bd_file):
    for link in links:
        driver.get(link)
        brand = driver.find_element("xpath", "//nav[@aria-label='breadcrumb']/ol/li[last()]").text
        cotegory = driver.find_element("xpath", "//nav[@aria-label='breadcrumb']/ol/li[@class='breadcrumb-item']/a").text
        colls = driver.find_elements("xpath", "//div[@id='collapseThree']//a/h2")
        colls = colls[:len(colls)//2]
        for coll in colls:
            ...
            coll_text = coll.get_attribute("textContent").strip()
            print(coll_text)
            insertColl(brand, coll_text, cotegory, bd_file)
